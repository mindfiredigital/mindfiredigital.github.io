...
/*
  React hook
  - useState: manages toggle state for accordion
*/
...
/*
  ...
  - Displays expandable scoring breakdown
  - Explains how contributor scores are calculated
  - Includes code, quality, and community metrics
*/
const ScoringSystem = () => {
  /* Toggle state for accordion open/close */
            ...
            <div className='grid grid-cols-1 sm:grid-cols-3 gap-4'>
              ...
              <div className='bg-blue-50 border border-blue-100 rounded-xl p-4'>
                ...
                <div className='space-y-2 text-xs text-blue-900/80 font-mono'>
                  {CODE_SCORE_ITEMS.map((item, idx) => (
                    <div key={idx} className='flex items-start gap-1.5'>
                      <span className='text-blue-400 mt-0.5'>+</span>
                      <span>
                        {item.label} × <strong>{item.multiplier}</strong>{" "}
                        {item.suffix ?? ""}
                      </span>
                    </div>
                  ))}
              ...
              <div className='bg-emerald-50 border border-emerald-100 rounded-xl p-4'>
                ...
                <div className='space-y-2 text-xs text-emerald-900/80 font-mono'>
                  {QUALITY_SCORE_ITEMS.map((item, idx) => (
                    <div key={idx} className='flex items-start gap-1.5'>
                ...
                <div className='space-y-2 text-xs text-violet-900/80 font-mono'>
                  {COMMUNITY_SCORE_ITEMS.map((item, idx) => (
                    <div key={idx}>
                      ...
                      {/* Cap display (if exists) */}
                      {item.cap && (
                        <div className='flex items-start gap-1.5'>
                          <span />
                          <span className='text-violet-400'>({item.cap})</span>
                        </div>
                      )}
                    </div>
                  ))}
