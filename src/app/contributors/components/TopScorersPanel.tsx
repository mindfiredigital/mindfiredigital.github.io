            ...
            {PODIUM_SLOTS.map((slot) => {
              ...
              return (
                ...
                >
                  {slot.rankIndex === 0 && (
                    <span
                      className='text-xl select-none mb-1 block'
                      style={{
        ...
        {rest.length > 0 && (
          <>
            <button
              ...
              className='w-full flex items-center justify-between px-4 py-2.5 hover:bg-gray-50 transition-colors duration-150 group'
            >
              <span className='text-[10px] font-bold text-gray-400 uppercase tracking-widest group-hover:text-gray-600 transition-colors'>
                {restOpen ? "Hide" : "Show"} ranks 4–
                {Math.min(10, rest.length + 3)}
              </span>
              <div className='flex items-center gap-1.5'>
                {!restOpen && (
                  <div className='flex -space-x-1.5'>
                    ...
                    {rest.length > 3 && (
                      <div className='w-5 h-5 rounded-full ring-1 ring-white bg-gray-100 flex items-center justify-center'>
                ...
                <div className='px-3 pb-3 flex flex-col gap-1'>
                  {rest.map((scorer, i) => {
                    ...
                    const pct = Math.round(
                      (scorer.total_score / maxScore) * 100
                    );
                    const accentClass =
